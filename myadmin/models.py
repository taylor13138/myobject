...
class User(models.Model):
    ...
    status = models.IntegerField(default=1)     #状态1、正常 2、禁用 6、管理员  9、删除
    create_at = models.DateTimeField(default=datetime.now)   #创建时间
    update_at = models.DateTimeField(default=datetime.now)   #修改时间

    #定义一个toDict(self)方法，返回当前数据对象信息的字典类型格式
    def toDict(self):
    ...
    class Meta:
        db_table = "user"   #更改表名   对应数据库中的user表
...
class Shop1(models.Model):
    ...
    def toDict(self):
        ...
        return {'id':self.id,'name':shopname[0],'shop':shopname[1],'status':self.status,'create_at':self.create_at.strftime('%Y-%m-%d %H:%M:%S'),'update_at':self.update_at.strftime('%Y-%m-%d %H:%M:%S')}
...
class Category(models.Model):
    shop_id = models.IntegerField()        #店铺id  今明后
    name = models.CharField(max_length=50)#分类名称 早中晚
...
class Product(models.Model):
    shop_id = models.IntegerField()        #店铺id  今明后
    category_id = models.IntegerField()    #菜品分类id  早中晚
...
class Member(models.Model):
    ...
    password_hash = models.CharField(max_length=50)    #密码
    password_salt = models.CharField(max_length=50)    #密码
    status = models.IntegerField(default=1)        #状态:1正常/2禁用/9删除
    create_at = models.DateTimeField(default=datetime.now)    #创建时间
    update_at = models.DateTimeField(default=datetime.now)    #修改时间

    def toDict(self):
        return {'id':self.id,'username':self.username,'nickname':self.nickname,'avatar':self.avatar,'mobile':self.mobile,'password_hash':self.password_hash,'password_salt':self.password_salt,'status':self.status,'create_at':self.create_at.strftime('%Y-%m-%d %H:%M:%S'),'update_at':self.update_at.strftime('%Y-%m-%d %H:%M:%S')}
